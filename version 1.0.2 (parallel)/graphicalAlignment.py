...
def parallelAligment(lengthX, seq1Array, lengthY, seq2Array, seqs_per_procs, strand, id):
    init = id*seqs_per_procs + id
    end = (id*seqs_per_procs + id) + seqs_per_procs
    if strand == 0:  # horizontal
        if end > lengthX:
            end = lengthX
        resultLocal = [[pairwise2.align.localmx(seq1Array[i], seq2Array[j], 5, -4, score_only=True) for j in range(lengthY)] for i in range(init, end)]
    else:  # vertical
        if end > lengthY:
            end = lengthY
...
def graphicalAlignment2(width, height, windowSize, name1, name2, graph_flag, result, lengthX, lengthY):
...
if __name__ == '__main__':
    ...
    if (joined_file1 != '' and joined_file2 != ''):
        lengthX, seq1Array = seq2array(file1, window)
        lengthY, seq2Array = seq2array(file2, window)
        ...
        localresults = [pool.apply_async(parallelAligment, args=(lengthX, seq1Array, lengthY, seq2Array, seqs_per_procs, strand, x)) for x in range(threads)]
        ...
        start_time = time.time()
        graphicalAlignment2(width, height, window, name1, name2, graph, result, lengthX, lengthY)
        end_time = time.time()
        print("module 3 time=", end_time - start_time)
    else:
        print("Please insert the sequences to align")
        print("Usage: -i <Sequence input file1> -a <Sequence input file2> -w WindowSize <int> -d <width> -e <height> -o <bool interactive mode> -t num_threads -help <More info>" )
        sys.exit(3)
