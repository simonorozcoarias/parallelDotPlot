...
def Distance(ref,subj,sizes_ref,sizes_subj,strand):
    ...
    #print(i,rank-1)
    command= "mashmap -r "+ref+"."+str(rank)+" -q "+str(subj)+".unique"+" -t 1 --pi 95 -s 500 -f none"+" -o mashmap.temp."+str(rank)
    #command= "mashmap -r "+ref+"."+str(rank)+" -q "+str(subj)+".unique"+" -t 1 --pi 95 -s 500 -k 5 -f none"+" -o mashmap.temp."+str(rank)
    #print(command)
    ...
    if joined_file1 != '' and joined_file2 != '':
        ...
        if rank == 0: 
            if strand==0:  # Esto quiere decir que la más larga es la horizontal, por lo tanto vamos a partir en Rank pedazos la vertical y la horizontal queda toda en un archivo.
                ...
                #namesX = CreateFiles(seqs_per_procs,seq1Array,lengthX,"horizontal",threads,n,strand)
                #namesY = CreateFiles(seqs_per_procs,seq2Array,lengthY,"vertical",threads,strand)
            else:
                namesx,namesY=CreateFiles(seqs_per_procs,seq1Array,seq2Array,lengthx,lengthy,"horizontal","vertical",threads,n)

            #print("Horizontal=",namesX,"vertical=",namesY)
            for m in range(1,threads):
