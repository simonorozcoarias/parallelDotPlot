...
def seq2array(file, windowSize):
    ...
    seqArray = wrap(str(te[0].seq), windowSize)
    return(len(seqArray), seqArray)
...
def parallelAligmentDNA(lengthX, seq1Array, lengthY, seq2Array, seqs_per_procs, strand, id):
    init = id*seqs_per_procs + id
    end = (id*seqs_per_procs + id) + seqs_per_procs + 1
    newEnd = (id*seqs_per_procs + id) + seqs_per_procs - 1
    newInit = id*seqs_per_procs + id - 1
    # print("init: "+str(init)+" end: "+str(end)+ " seqs_per_procs: "+str(seqs_per_procs)+" id: "+str(id))
    if strand == 0:  # horizontal
        if end > lengthX:
            end = lengthX
            newEnd = lengthX - 1
        # direct score
        resultLocal = [[scoreFunction(seq1Array[i], seq2Array[j], 5, -4) for j in range(lengthY)] for i in range(init, end)]
        ...
        for x in range(init, end):
           for y in range(lengthY):
              reverseScore = scoreFunction(seq1Array[x], ''.join(reversed([complementary(n) for n in seq2Array[y]])), 5, -4)
              if reverseScore > resultLocal[i][j]:
                 resultLocal[i][j] = reverseScore
              j += 1
           j = 0
           i += 1
    else:  # vertical
        if end > lengthY:
            ...
            newEnd = lengthY - 1
        # direct score
        resultLocal = [[scoreFunction(seq1Array[i], seq2Array[j], 5, -4) for j in range(init, end)] for i in range(lengthX)]
...
def scoreFunction(seq1, seq2, matScore, misScore):
    ...
    #score = numMis*misScore + (length-sum(mismatches))*matScore #Funcion de score usando tanto matches como mismatches
    score = (length-sum(mismatches))*matScore # funcion de score usando solo los matches
    #if score < 0:
    #   score = 0

    # score = pairwise2.align.localmx(seq1, seq2, matScore, misScore, score_only=True, one_alignment_only = True)
    # score for reverse
    """newseq2 = ''.join([complementary(n) for n in seq2])
    mismatches = [1 for x in range(length) if seq1[x] != newseq2[x]]
    numMis = sum(mismatches)# + abs(len(seq1) - len(newseq2))
    scoreR = (numMis*misScore + (length-sum(mismatches))*matScore)/length
    if scoreR < 0:
       scoreR = 0
    #print("Score: %d, ScoreR: %d"%(score, scoreR))
    if score > scoreR:
       return score
    else:
       return scoreR"""
...
if __name__ == '__main__':
	...
	# creating the multiprocessing environment
        ...
        print("Usage: -i <Sequence input file1> -a <Sequence input file2> -w WindowSize <int> -d <width> -e <height> -o <bool interactive mode> -t num_threads <int> -help <More info>" )
        sys.exit(3)
